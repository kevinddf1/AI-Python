...
class ClueGameReasoner:
    ...
    def __init__(self, player_order, card_nums = None):
        ...
        def atLeast(cArr, p): ## cArr stands for card Array, p stands for position
            clause = ""
            for x in cArr :
                clause += x + "_" + p + " "
            clauses.append(clause)
    ...
    def suggest(self, suggester, c1, c2, c3, refuter, cardshown = None):
        ...
        if (refuter==None):
            for l in self.players:
                if(l!=suggester):
                    self.KB.add_clause("~"+c1+"_"+l)
                    self.KB.add_clause("~"+c2+"_"+l)
                    self.KB.add_clause("~"+c3+"_"+l)
        else:
            indexS=self.players.index(suggester)
            indexR=self.players.index(refuter)
            List=[]
            if(indexS<indexR):
                List=self.players[indexS+1: indexR]
            else:
                List=self.players[indexS+1: self.players.__len__()]+self.players[0: indexR]
            for l in List:
                self.KB.add_clause("~"+c1+"_"+l)
                self.KB.add_clause("~"+c2+"_"+l)
                self.KB.add_clause("~"+c3+"_"+l)
